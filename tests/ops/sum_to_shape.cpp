...
TEST(SumToShape, Float32) {
  ...
  quasai::Shape target{3};

  quasai::Tensor result = quasai::sum_to_shape(tensor, target);
...
TEST(SumToShape, Broadcast) {
  ...
  quasai::Tensor tensor =
      quasai::Tensor::from_data(data.data(), shape, quasai::DType::FLOAT32);
  quasai::Shape target{2, 3};

  EXPECT_THROW(quasai::sum_to_shape(tensor, target), std::runtime_error);
}
